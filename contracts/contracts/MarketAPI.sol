...
contract MarketAPI {
    ...
    mapping(string => uint64) deals_by_provider;
    uint64 size;
    ...
    function mock_get_auto_rpovider()
        external
        view
        returns (string memory provider, uint256 price_per_epoch)
    {
        if (size == 0) {
            return ("", 0);
        }

        uint64 id = _random(0, size);
        MarketTypes.MockDeal memory deal = deals[id];

        provider = deal.provider;
        price_per_epoch = deal.price_per_epoch;
    }

    function _random(uint64 minNumber, uint64 maxNumber)
        internal
        view
    ...
    function mock_provider_deal(string calldata provider)
        external
        view
        returns (MarketTypes.MockDeal memory)
    {
        return deals[deals_by_provider[provider]];
    }
}
